          ...
          if (resizedDetections.length > 0) {
            ...
            setHudData({
              ...
              detection: {
                ...
                box: { x: topDetection.detection.box.x, y: topDetection.detection.box.y }
              }
            });
          } else {
            setHudData(null);
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (activeTab === 'live_camera' && videoRef.current) {
        ...
        throw new Error(`Please upload a file for ${activeTab.replace('_', ' ')} analysis.`);
      }

      const { data } = await postWithApiFallback('/scan/detect', formData);
      setResult(data);
      await fetchHistory();
      ...
      Promise.all([jsPDFModule, autoTableModule]).then(([jspdf, autotable]) => {
        ...
        if (result.comparative_analysis && result.comparative_analysis.length > 0) {
          autotable.default(doc, {
            ...
            body: result.comparative_analysis.map(row => [safeText(row.metric), safeText(row.observed), safeText(row.benchmark), safeText(row.status)]),
            theme: 'grid',
        ...
        <div className="col-span-12 lg:col-span-3 space-y-6">
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 h-[500px] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2">
              {history.length === 0 && (
                <p className="text-gray-600 text-sm italic text-center mt-8">No scans yet. Run your first analysis!</p>
              )}
              {history.map(scan => (
            ...
            <div className="min-h-[200px] border-2 border-dashed border-neutral-800 rounded-xl flex flex-col items-center justify-center bg-neutral-950 relative">
              ...
              {activeTab === 'live_camera' && (
                <div className="w-full h-full min-h-[400px] flex flex-col items-center justify-center relative overflow-hidden rounded-lg bg-black group">
                  ...
                  <div className="absolute top-8 left-8 w-16 h-16 border-t-4 border-l-4 border-yellow-400 z-20 pointer-events-none opacity-80"></div>
                  <div className="absolute top-8 right-8 w-16 h-16 border-t-4 border-r-4 border-yellow-400 z-20 pointer-events-none opacity-80"></div>
                  <div className="absolute bottom-8 left-8 w-16 h-16 border-b-4 border-l-4 border-yellow-400 z-20 pointer-events-none opacity-80"></div>
                  <div className="absolute bottom-8 right-8 w-16 h-16 border-b-4 border-r-4 border-yellow-400 z-20 pointer-events-none opacity-80"></div>

                  {modelsLoaded && hudData && (
                    <div className="absolute inset-y-0 right-4 flex items-center justify-center z-30 pointer-events-none">
                      <div className="flex flex-col gap-2 font-mono text-cyan-400 text-xs tracking-widest bg-black/40 p-4 rounded-lg backdrop-blur-md border border-cyan-500/30 w-56 uppercase">
                        ...
                        <p className="mt-2 text-yellow-400/80">POS: {Math.round(hudData.detection.box.x)} / {Math.round(hudData.detection.box.y)}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {modelsLoaded && (
                    <div className="absolute top-4 right-4 z-30 bg-black/60 backdrop-blur border border-white/10 p-3 rounded-xl flex flex-col items-center opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <input
                        type="range" min="1" max="3" step="0.1" value={cameraZoom}
                  ...
                  <video
                    ...
                    style={{ transform: `scale(${cameraZoom})`, transformOrigin: 'center center', transition: 'transform 0.1s ease-out' }}
                  />
                  <canvas
                    ref={canvasRef}
                        ...
                        {result.web_detection.exact_matches?.length > 0 ? (
                          <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                            {result.web_detection.exact_matches.map((url, i) => (
                              <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block text-xs text-gray-300 hover:text-white truncate bg-neutral-950 p-2 rounded border border-neutral-800 hover:border-red-500/50 transition">{url}</a>
                            ))}
                          </div>
                        ) : (
                        ...
                        {result.web_detection.partial_matches?.length > 0 ? (
                          <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                            {result.web_detection.partial_matches.map((url, i) => (
                              <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block text-xs text-gray-300 hover:text-white truncate bg-neutral-950 p-2 rounded border border-neutral-800 hover:border-amber-500/50 transition">{url}</a>
                            ))}
                          </div>
                        ) : (
